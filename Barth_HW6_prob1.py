...
if diff_method == "RK":
    for t in tdata:
        ...
        r_0 += (k1 + k2 + k3 + k4)/6
#solce diff eq by Leap Frog Method
if diff_method == "LF": #doesnt work every time
    i = 0
    for t in tdata:
        rarray.append(r_0)
        r_0[0] = r_0[0] + r_0[1]*t + 0.5*((numpy.pi/180)*(-9.81)/(length) * numpy.cos(r_0[0]*(numpy.pi / 180.)))*(t**2)
        thetadata.append(r_0[0])
        r_0[1] = r_0[1] + 0.5*((numpy.pi/180)*(-9.81)/(length) * numpy.cos(rarray[i][0]*(numpy.pi / 180.))+(numpy.pi/180)*(-9.81)/(length) * numpy.cos(r_0[0]*(numpy.pi / 180.)))*t
        omegadata.append(r_0[1])
        i += 1
